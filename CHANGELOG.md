...
- `irl-paperless` v0.2.0: Bump paperless-ngx from 2.14.7 to 2.20.14. Enables API v9 support (required by current MCP server and API clients) and pulls in all security fixes across the 2.15.x-2.20.x line. Updates the image tag in both the main paperless deployment and the backup CronJob (which reuses the paperless image for `document_exporter`). No custom filename templates or storage path templates are in use on this instance, so the stricter rendering introduced in 2.20.7 has no impact. No subchart bump required (gabe565/paperless-ngx stays at 0.24.1).
